...
def load_tv_df(annotations_file_path, images_path):
    ...
    random.seed(42)
    validation_files = set(random.sample(file_names, int(len(file_names) * 0.2)))
...
class TvDatasetAdaptor(Dataset):
    def __init__(
        self,
        images_dir_path,
        ...
        for idx, image_id in enumerate(image_ids):
            ...
            image_info = df[df.image_id == image_id]
...
DATA_PATH = Path("/".join(Path(__file__).absolute().parts[:-2])) / "data/subset-coco"
...
def main(
    ...
    images_path = data_path / "jpg-train"
